...
async def generate(request: Request, rec_date: Optional[date] = Query(None, alias="date"), db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    """手动触发生成指定日期的量化推荐"""
    target_date = rec_date or date.today()
...
async def update_prices(request: Request, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    """更新所有推荐股票的最新价格"""
...
async def delete_item(rec_id: int, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    """删除一条推荐记录"""
...
async def reset_item(rec_id: int, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
...
async def update_item(request: Request, rec_id: int, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    """单独更新一条推荐的价格"""
...
async def batch_reset(ids: list[int] = Body(...), db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    """批量重置多条推荐的收益跟踪"""
...
async def batch_delete(ids: list[int] = Body(...), db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    """批量删除多条推荐记录"""
