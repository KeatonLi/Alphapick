...
def ensure_runtime_schema():
    ...
    if "schedule_config" in tables:
        existing_schedule = {col["name"] for col in inspector.get_columns("schedule_config")}
        if "run_update_returns" not in existing_schedule:
            with engine.begin() as conn:
                if dialect == "mysql":
                    conn.execute(text("ALTER TABLE schedule_config ADD COLUMN run_update_returns BOOL DEFAULT TRUE COMMENT '是否自动更新收益'"))
                else:
                    conn.execute(text("ALTER TABLE schedule_config ADD COLUMN run_update_returns BOOLEAN DEFAULT 1"))
